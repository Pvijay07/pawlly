...
use Modules\Booking\Models\BookingList;
...
class BookingsController extends Controller
{
    ...
    {
        $users_data = User::where('id', auth()->user()->id)->first();
        ...
        $usersAdd = User::where('address', $users_data->address)
            ->where('user_type', $type_data)
            ->get();

//             //  return response()->json(['message' => $message, 'status' => true, 'booking_id' =>$users], 200);
//             //  exit();
            // $booking = null;
...
foreach ($usersAdd as $user) {
        ...
        if($i==1){
            //$mdata = $data;
            $mdata['id'] = '';
            $mdata['employee_id'] = null;
            $mdata['note'] = $data['note'] ?? '';
            $mdata['status'] = $data['status'] ?? 'pending';
            $mdata['start_date_time'] = $data['start_date_time'];
            $mdata['user_id'] = $data['user_id'];
            $mdata['branch_id'] = $data['branch_id'];
            $mdata['system_service_id'] = $data['system_service_id'];
            $mdata['pet_id'] = $data['pet_id'];
            $mdata['booking_extra_info'] = $data['booking_extra_info'] ?? '';
            $mdata['booking_type'] = $data['booking_type'];
            $mdata['total_amount'] = $data['total_amount'];
            $mdata['service_amount'] = $data['service_amount'];
            $mbooking = BookingList::updateOrCreate($mdata);
            $data['booking_list_id'] = $mbooking->id;
            $mbooking['booking_list_id'] = $mbooking->id;
            //echo "<pre>"; print_r($data); print_r($mbooking); exit();
        }else{
            if(isset($mbooking['booking_list_id'])){
                $data['booking_list_id'] = $mbooking['booking_list_id'];
            }
        }
        ...
        try {
            ...
            if($user->deviceid){
                $notificationData['device_id'] = $user->deviceid;
                $notificationData['title'] = 'New service '.$request->booking_type;
                $notificationData['message'] = 'New Service '.$request->booking_type.' requested by '.optional($booking->user)->first_name ?? default_user_name();
        ...
        return response()->json(['message' => $message, 'status' => true, 'booking_id' => $booking->id,'user'=>$usersAdd], 200);
    }else
    {
        return response()->json(['message' => "Dr. Not found", 'booking_id' => '','user'=>'','status' => false, ], 200);
    }
}
    ...
    public function bookingList(Request $request)
    {
        ...
        $booking->whereNotNull('booking_list_id');
        ...
        if ($request->has('per_page') && !empty($request->per_page)) {
            if ($request->per_page === 'all') {
                $per_page = $booking->count(); // This will likely need to be updated as it might not perform as expected.
            }
        }
...
// Group by booking_list_id before ordering and paginating
        //$booking = $booking->groupBy('booking_list_id');
        $booking = $booking->orderBy('updated_at', $orderBy);
       // print_r($booking->get()); die();
        // Use paginate method
        $items = BookingListResource::collection($booking->paginate($per_page));
    ...
    public function bookingUpdate(Request $request)
    {
        ...
        $booking = Booking::findOrFail($id);
        $parentBooking = $booking->getAttribute('booking_list_id');
        $originalBookings = Booking::where('booking_list_id',$parentBooking)->whereNot('id',$id)->get();
        $bookingPreviousStatus =  $booking->getAttribute('status');
        if($bookingPreviousStatus == 'interested' && $data['status'] == 'confirmed'){
            foreach($originalBookings as $originalBooking){
                $mydata = [];
                $bookingId = $originalBooking->getAttribute('id');
                $mydata['id'] = $bookingId;
                $mydata['status'] = 'rejected';
                $booking1 = Booking::findOrFail($bookingId);
                $booking1->fill($mydata);
                $booking1->save();
                $this->updateServices($request,$booking1,$mydata,$bookingId);
            }
        }
        $booking->fill($data);
    ...
    public function updateServices($request,$booking,$data,$id)
    {
        if($booking->booking_type === 'boarding'){
