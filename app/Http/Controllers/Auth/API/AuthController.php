...
class AuthController extends Controller
{
    ...
    {
        // Example check (implement your OTP validation logic)
        return $user->otp == $otp;
    }
    ...
    public function sendSMS($mobile_number, $message, $template_id)
    {
        ...
        $params = http_build_query([
            ...
            'message' => $message,
            'sender' => $sender,
            'type' => '3', // Assuming type is constant
            'template_id' => $template_id
        ]);
        $url = "http://api.bulksmsgateway.in/sendmessage.php?" . $params;
        $ch = curl_init($url);
    ...
    public function requestOtp(Request $request)
    {
        ...
        if ($user->is_banned || $user->status == 0) {
            return response()->json(['status' => false, 'message' => __('messages.user_banned')]);
        }
        if ($user) {
            $mobileNumbers = ['8367678910','9731747471'];
            if(in_array($request->mobile,$mobileNumbers)){
                $otp = '1234';
            }else{
                $otp = $this->generateOtp();
            }
            $message = 'Dear Customer,Your OTP number is ' . $otp . ',Regards PETSFOLIO.';
            $mobile_number = $request->mobile;
            $template_id = '1207168240213327862';
            $user->otp = $otp;
            ...
            $user->save();
            $responseData = $this->sendSMS($mobile_number, $message, $template_id);
            $smsResponse = json_decode($responseData,true);
    ...
    public function mobileLogin(Request $request)
    {
        ...
        if (!$this->verifyOtp($user, $request->otp)) {
            return response()->json(['status' => false, 'message' => __('messages.invalid_otp')]);
        }
        ...
        if($user->deviceid){
            ...
            $notificationData['title'] = 'User Login';
            $notificationData['message'] = 'Successfully login to your account.';
